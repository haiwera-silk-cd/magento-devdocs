...
A custom UI component can be declared using one of the following elements: `<component>`或`<container>`.
...
If the following elements are used inside `<container>`或`<component>`, they should be specified strictly in the following order:
