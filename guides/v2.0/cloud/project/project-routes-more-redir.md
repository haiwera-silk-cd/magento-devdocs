...
### `to` {#cloud-route-partial-to}
Required, a partial (`"/destination"`或`"//destination"`) or full URL (`"http://example.com/"`).
...
### `append_suffix` {#cloud-route-partial-append}
Determines if the suffix is carried over with the redirect. Defaults to `true`, but not supported if `regexp` is `true` *或* if `prefix` is `false`.
