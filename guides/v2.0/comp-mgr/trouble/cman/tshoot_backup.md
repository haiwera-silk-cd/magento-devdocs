...
If the backup fails or if all backup tests fail, it's possible the <a href="{{ page.baseurl }}/install-gde/prereq/apache-user.html">Magento文件系统所有者</a> doesn't have sufficient privileges and ownership of the Magento file system. 例如， another user might own the files or the files might be read-only.
