...
{% highlight xml %}
{% include magento/magento2/2.0/dev/tests/functional/phpunit.xml.dist %}
{% endhighlight %}
{% endcollapsible %}
...
{% highlight xml %}
{% include magento/mtf/2.0/etc/config.xml.dist%}
{% endhighlight %}
...
{% highlight xml %}
{% include magento/magento2/2.0/dev/tests/functional/etc/config.xml.dist %}
{% endhighlight %}
...
|`<baseUrl>`|Magento application base URL, starting with the scheme (`http://`或`https://`) and ending with a slash character (`/`).|
