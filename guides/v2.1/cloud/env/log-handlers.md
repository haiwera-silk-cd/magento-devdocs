...
The following example configures both of these handlers by adding the configuration to the `.magento.env.yaml` file. For the minimum logging level (`min_level`), see [设置通知—Log levels]({{ page.baseurl }}/cloud/env/setup-notifications.html#log-levels).
