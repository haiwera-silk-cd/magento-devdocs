...
![Web API request processing overview]({{ site.magentourl }}/common/images/coding-standards/webapi-request-processing-high-level-overview.png)
...
1. Unlimited nesting should be supported during requests for related entities. (例如， get Order => Order Items => Products => Related Products)
...
1. For modularity purposes, GraphQL configuration must be declared in a separate module. 例如， to expose GraphQL for the module `MyModule`, you must create the `schema.graphqls` file in the `MyModuleGraphQl` module.
