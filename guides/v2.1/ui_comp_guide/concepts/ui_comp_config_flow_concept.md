...
Lets imagine we have the following file structure in our {% glossarytooltip c1e4242b-1f1a-44c3-9d72-1d5b1435e142 %}模块{% endglossarytooltip %} <My_Module>:

- {% glossarytooltip 73ab5daa-5857-4039-97df-11269b626134 %}布局{% endglossarytooltip %} `.xml` file of the Module’s page: `my_page.xml`
...
1. Determines which UI 组件 are used in this particular layout. In the example, the UI 组件 that are used are defined in the `my_form` component’s `.xml` declaration file.
...
1. Merges the resulting configuration (from 步骤2.above) with the configuration from the UI module `definition.xml`. The UI module `definition.xml` configuration file has the lowest priority, and is overwritten by the merged configuration of all `my_form.xml` files.
...
3. `layout.js` creates instances of UI 组件. That means that each UI component’s configuration must have an explicitly declared `component` property in JSON. This property references the `.js` file. For example, our form has the component declared in JSON like this:	`"my_form":{"component":"Magento_Ui/js/form/form"}`
...
5. The `bootstrap.js` binds the component as a Model behind this View (template) using Knockout bindings. The UI 组件 are now displayed on the page, and are fully interactive.
