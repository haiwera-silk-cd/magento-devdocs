...
* Set up SSH keys as the [Magento文件系统所有者]({{ page.baseurl }}/cloud/before/before-workspace-file-sys-owner.html).
...
2.	If you haven't done so already, log in (or switch to) the [Magento文件系统所有者]({{ page.baseurl }}/cloud/before/before-workspace-file-sys-owner.html) to the server on which your SSH keys are located.
...
3.	In your selected branch, an icon displays if you do not have an SSH key added.
	...
	![No SSH key]({{ site.magentourl }}/common/images/cloud_ssh-key-install.png)
4.	Copy and paste the content of your public SSH key in the screen.

	![Add SSH key]({{ site.magentourl }}/common/images/cloud_ssh-key-add.png)
